...
namespace LDM.Module.Web.Controllers.CheckBoxCustomization
{
    ...
    public partial class GroupRowCheckBoxForCOCSettingsTest : ViewController<ListView>
    {
        #region Declaration
        public const string keyFieldName = "Oid";
        public const string gridId = "COCSettingsTest_ListView_Copy_SampleRegistration";
        MessageTimer timer = new MessageTimer();
        #endregion

        #region Constructor
        public GroupRowCheckBoxForCOCSettingsTest()
        {
            InitializeComponent();
            // Target required Views (via the TargetXXX properties) and create their Actions.
            TargetViewId = "COCSettingsTest_ListView_Copy_SampleRegistration;";
        ...
        protected void Grid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            try
            {
                ...
                if (groupedCols[groupedCols.Count - 1].FieldName == fieldname)
                {
                    ...
                    {
                        var key = (Grid.GetChildRow(index, childIndex) as COCSettingsTest).Oid;
                ...
                else
                {
                    ...
                    COCSettingsTest checkedDataRow = Grid.GetRow(index) as COCSettingsTest;
                    ...
                    {
                        COCSettingsTest row = Grid.GetRow(i) as COCSettingsTest;
        ...
        protected bool GetChecked(int visibleIndex)
        {
            try
            {
                for (int childIndex = 0; childIndex < Grid.GetChildRowCount(visibleIndex); childIndex++)
                {
                    ...
                    var key = (Grid.GetChildRow(visibleIndex, childIndex) as COCSettingsTest).Oid;
