...
namespace LDM.Module.Web.Controllers.CheckBoxCustomization
{
    ...
    public partial class GroupRowCheckBoxForTestParameter : ViewController<ListView>
    {
        #region Declaration
        public const string keyFieldName = "Oid";
        private System.ComponentModel.IContainer components;
        public const string gridId = "SampleLogIn_Testparameters_ListView";
        MessageTimer timer = new MessageTimer();
        #endregion

        #region Constructor
        public GroupRowCheckBoxForTestParameter()
        {
            InitializeComponent();
            // Target required Views (via the TargetXXX properties) and create their Actions.
            TargetViewId = "SampleLogIn_Testparameters_ListView;";
        ...
        protected void Grid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            try
            {
                ...
                if (groupedCols[groupedCols.Count - 1].FieldName == fieldname)
                {
                    ...
                    {
                        var key = (Grid.GetChildRow(index, childIndex) as Testparameter).Oid;
                ...
                else
                {
                    ...
                    Testparameter checkedDataRow = Grid.GetRow(index) as Testparameter;
                    ...
                    {
                        Testparameter row = Grid.GetRow(i) as Testparameter;
        ...
        protected bool GetChecked(int visibleIndex)
        {
            try
            {
                for (int childIndex = 0; childIndex < Grid.GetChildRowCount(visibleIndex); childIndex++)
                {
                    ...
                    var key = (Grid.GetChildRow(visibleIndex, childIndex) as Testparameter).Oid;
