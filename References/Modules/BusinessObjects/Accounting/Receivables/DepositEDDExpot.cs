...
{
    [DefaultClassOptions]
    public class DepositEDDExport : BaseObject
    { // Inherit from a different class to provide a custom primary key, concurrency and deletion behavior, etc. (https://documentation.devexpress.com/eXpressAppFramework/CustomDocument113146.aspx).
        public DepositEDDExport(Session session)
            : base(session)
        {
        }
        ...
        protected override void OnSaving()
        {
            if (string.IsNullOrEmpty(EDDID))
            {
                ...
                string tempID = (Convert.ToInt32(Session.Evaluate(typeof(DepositEDDExport), criteria, null)) + 1).ToString();
                if (tempID != "1")
                {
                    tempID = "DED" + tempID;
                }
        ...
        #endregion
        #region DepositID
        private Deposits _DepositID;
        public Deposits DepositID
        {
